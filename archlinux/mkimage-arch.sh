...
expect <<EOF
	...
	spawn pacstrap -C $PACMAN_CONF -c -d -G -i $ROOTFS ${PKGLIST[@]} $PACMAN_EXTRA_PKGS
	expect {
...
LCOV_PKG="lcov-1.12-1-any.pkg.tar.xz"
cp $LCOV_PKG $ROOTFS
arch-chroot $ROOTFS /bin/sh -c "pacman -U --noconfirm $LCOV_PKG; rm $LCOV_PKG"
...
tar --numeric-owner --xattrs --acls -C $ROOTFS -c . -f archlinux.tar
